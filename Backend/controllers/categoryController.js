...
export class CategoryController {
  ...
  async createCategory(req, res) {
    ...
    if (!category_name) {
      return res
        .status(400)
        .json({ message: 'el nombre de la categoria es obligatoria' });
    }
  ...
  async updateCategory(req, res) {
    ...
    if (!id_category || !category_name) {
      return res
        .status(400)
        .json({ message: 'el nombre y el id de la categoria es obligatoria ' });
    }
  ...
  async deleteCategoryById(req, res) {
    ...
    if (!id_category) {
      return res
        .status(400)
        .json({ message: 'el  el id de la categoria es obligatoria ' });
    }
    const deleteCategory = await categoryService.deleteCategoryById(
      id_category
    );
