...
export class ShoppingCarController {
    ...
    try {
      ...
      if (!id_product_details || !quantity || !id_user || !price) {
        return res
          ...
          .json({
            message:
              "Los campos 'id_product_details', 'quantity', 'id_user' y 'price' son obligatorios",
          });
      }

      const total_price = quantity * price;
      const date = new Date().toISOString();
      const shoppingCar = await shoppingCarService.createShoppingCar(
        id_user,
        date,
        total_price
      );
      ...
      if (!findItemState) {
        return res
          .status(500)
          .json({
            message: "No se pudo encontrar el estado del artículo activo.",
          });
      }
  ...
  async getShhoppingCar(req, res) {
    try {
      ...
      if (!id_user) {
        return res
          .status(400)
          .json({ message: "El campo 'id_user' es obligatorio" });
      }
      const carItems = await shoppingCarService.getAllShoppingCarByIdUser(
        id_user
      );
      ...
      return res
        .status(500)
        .json({ message: "Error al obtener los productos del carrito" });
    }
  }
  async deleteCartItem(req, res) {
    try {
      const { id_shopping_cart_item } = req.params;

      if (!id_shopping_cart_item) {
        return res
          .status(400)
          .json({ message: "Se requiere el id del ítem para eliminarlo" });
      }

      const deletedItem = await shoppingCarService.deleteCartItem(
      ...
      if (!deletedItem) {
        return res
          .status(404)
          .json({ message: "El producto no fue encontrado en el carrito" });
      }
  ...
  async updateCartItemQuantity(req, res) {
    try {
      const { id_shopping_cart_item, quantity } = req.body;

      if (!id_shopping_cart_item || !quantity) {
        return res
          .status(400)
          .json({ message: "Faltan datos para actualizar la cantidad" });
      }

      const updatedItem = await shoppingCarService.updateCartItemQuantity(
        id_shopping_cart_item,
        quantity
      );
      ...
      if (!updatedItem) {
        return res
          .status(404)
          .json({ message: "No se encontró el producto en el carrito" });
      }

      return res
        .status(200)
        .json({ message: "Cantidad actualizada correctamente", updatedItem });
    } catch (err) {
